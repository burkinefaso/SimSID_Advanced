...
class GanAlert(object):
    def __init__(self, discriminator, args, CONFIG=None, generator=None):
        ...
        self.CONFIG.lpips_fn = lpips.LPIPS(net='alex').cuda()
    ...
    def get_hybrid_score(self, img, recon):
        with torch.no_grad():
            mse_score = F.mse_loss(recon, img, reduction='none')
            mse_score = mse_score.view(mse_score.size(0), -1).mean(dim=1)

            lpips_score = self.CONFIG.lpips_fn(recon, img).view(-1)

            disc_score = self.discriminator(recon)
            disc_score = disc_score.view(disc_score.size(0), -1).mean(dim=1)

            # Ağırlıklar burada ayarlanabilir
            α = 0.4
            β = 0.4
            γ = 0.2

            hybrid_score = α * mse_score + β * lpips_score + γ * disc_score
            return hybrid_score
    ...
    def alert(self, scores, labels, mean=0., std=1., print_info=True):
        ...
        labels = np.array(labels)
                # ROC için skorları ve etiketleri 1D yap
        if scores.ndim > 1:
