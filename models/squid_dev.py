...
class AE(nn.Module):
    def __init__(self, num_in_ch, features_root, shrink_thres, 
        ...
        self.memory_blocks = nn.ModuleList([
            ...
            MemoryMatrixBlock(mem_num_slots, self.filter_list[level-2]) if ops[1] != 'none' else nn.Identity(),
            MemoryMatrixBlock(mem_num_slots, self.filter_list[level-1]) if ops[0] != 'none' else nn.Identity(),
        ])
