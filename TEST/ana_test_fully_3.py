...
RESULTS_BASE = r"C:\Users\BURKINEFASO\LPIPS_SIMSID\FRIKKO_1_TEST_RESULTS"
NORM_PER_SET = 1000
ANOM_PER_SET = 1000
...
# ----------- GÜNCELLENMİŞ GÖRSEL FONKSİYONLARI -----------
...
def plot_histogram_kde(scores, labels, threshold, save_dir, prefix):
    ...
    # Histogram (a)
    plt.subplot(1,2,1)
    counts_normal, bins_normal, _ = plt.hist(
        scores[labels==0], bins=bin_count, alpha=0.8,
        label=f"Normal ({n_normal})", color="royalblue", density=False
    )
    counts_anom, bins_anom, _ = plt.hist(
        scores[labels==1], bins=bin_count, alpha=0.7,
        label=f"Anomali ({n_anom})", color="red", density=False
    )
    plt.axvline(threshold, color="purple", linestyle="--", linewidth=2, label=f"Eşik = {threshold:.2f}")
    plt.title("Anomali Skoru Histogramı", fontsize=13, fontweight="bold")
    plt.xlabel("Anomali Skoru")
    plt.ylabel("Kişi Sayısı")
    plt.legend(loc="upper right")
    plt.text(0.98, max(max(counts_normal), max(counts_anom))*0.95, 
             "Her 1 birim = 1 kişi", ha="right", va="top", fontsize=9, color="dimgray")
    # Alt açıklama (a)
    plt.text(0.5, -0.23, "(a) Anomali Skoru Histogramı", ha="center", va="center", fontsize=12, transform=plt.gca().transAxes)
    # KDE (b)
    plt.subplot(1,2,2)
    sns.kdeplot(scores[labels==0], label=f"Normal ({n_normal})", fill=True, color="royalblue", linewidth=1.8, alpha=0.4, bw_adjust=1)
    sns.kdeplot(scores[labels==1], label=f"Anomali ({n_anom})", fill=True, color="red", linewidth=1.8, alpha=0.4, bw_adjust=1)
    plt.axvline(threshold, color="purple", linestyle="--", linewidth=2, label=f"Eşik = {threshold:.2f}")
    plt.title("KDE Yoğunluk Grafiği", fontsize=13, fontweight="bold")
    plt.xlabel("Anomali Skoru")
    # KDE y ekseni: kişi sayısı ile ölçekli (yaklaşık)
    locs, labels_ = plt.yticks()
    plt.yticks(locs, [f"{int(round(y * len(scores)))}" for y in locs])
    plt.ylabel("Kişi Sayısı (Tahmini)")
    plt.legend(loc="upper right")
    plt.text(0.98, locs[-1]*0.95, "Y ekseni kişi sayısı ile ölçeklendi", ha="right", va="top", fontsize=9, color="dimgray")
    # Alt açıklama (b)
    plt.text(0.5, -0.23, "(b) KDE Yoğunluk Grafiği", ha="center", va="center", fontsize=12, transform=plt.gca().transAxes)
    plt.tight_layout()
...
def plot_roc_prc_cm(y_true, scores, save_dir, threshold):
    ...
    axs[0].set_title("ROC Eğrisi\nAUC={:.3f}".format(roc_auc), fontsize=13, fontweight="bold")
    axs[0].set_xlabel("Yanlış Pozitif Oran")
    axs[0].set_ylabel("Doğru Pozitif Oran")
    axs[0].text(0.5, -0.18, "(a)", ha="center", va="center", transform=axs[0].transAxes, fontsize=13)
    # PRC
    axs[1].plot(recall, precision, color="green", linewidth=2)
    axs[1].set_title("Kesinlik-Duyarlılık Eğrisi\nAUC={:.3f}".format(prc_auc), fontsize=13, fontweight="bold")
    axs[1].set_xlabel("Geri Çağırma")
    axs[1].set_ylabel("Kesinlik")
    axs[1].text(0.5, -0.18, "(b)", ha="center", va="center", transform=axs[1].transAxes, fontsize=13)
    # CM
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=["Normal", "Anomali"], yticklabels=["Normal", "Anomali"], ax=axs[2])
    axs[2].set_title("Karışıklık Matrisi", fontsize=13, fontweight="bold")
    axs[2].text(0.5, -0.18, "(c)", ha="center", va="center", transform=axs[2].transAxes, fontsize=13)
    plt.tight_layout()
...
def plot_umap_kmeans_pca_tsne(features, labels, save_dir):
    ...
    axs[0,0].set_title("UMAP", fontsize=13, fontweight="bold")
    axs[0,0].legend()
    axs[0,0].text(0.5, -0.15, "(a)", ha="center", va="center", transform=axs[0,0].transAxes, fontsize=13)
    axs[0,1].scatter(X_umap[:,0], X_umap[:,1], c=kmeans_labels, cmap="coolwarm", alpha=0.7, s=10)
    axs[0,1].set_title("K-Means Kümeleme (UMAP)", fontsize=13, fontweight="bold")
    axs[0,1].text(0.5, -0.15, "(b)", ha="center", va="center", transform=axs[0,1].transAxes, fontsize=13)
    ...
    axs[1,0].set_title("PCA", fontsize=13, fontweight="bold")
    axs[1,0].legend()
    axs[1,0].text(0.5, -0.15, "(c)", ha="center", va="center", transform=axs[1,0].transAxes, fontsize=13)
    ...
    axs[1,1].set_title("t-SNE", fontsize=13, fontweight="bold")
    axs[1,1].legend()
    axs[1,1].text(0.5, -0.15, "(d)", ha="center", va="center", transform=axs[1,1].transAxes, fontsize=13)
    plt.tight_layout()
